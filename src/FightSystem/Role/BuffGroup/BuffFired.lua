...
BuffFired = class("BuffFired")

function BuffFired:ctor(_role, _buffCon, _releaser, _dbState, _dbEff, _level, _processType, _subhost)
	self.mRole = _role
	...
	--FightSystem:PushNotification("fireddamage")
	if self.mRole.mFEM:IsFrozen() then
		self.mRole.mBeatCon.mFrozenCon:firedInFrozen()
	end
	self.mBuffCon:addLightEffect(_dbState,self)
end

function BuffFired:Destroy()
	self.mRole = nil
	self.mBuffCon = nil
	self.mReleaser = nil
	self.mEffMethod = nil
