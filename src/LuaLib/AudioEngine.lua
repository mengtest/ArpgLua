...
function M.playEffect(filename, isLoop, pitch)
    local loopValue = false
    if nil ~= isLoop then
        loopValue = isLoop
    end
    if pitch then
        return cc.SimpleAudioEngine:getInstance():playEffect(filename, loopValue, pitch)
    else
...
function M.unloadAllEffects()
    cc.SimpleAudioEngine:getInstance():unloadAllEffects()
end
