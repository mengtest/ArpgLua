...
TaskGuideOne.level = 13

-- 指引层
TaskGuideOne.guideLayer = nil

-- 处于指引过程中
TaskGuideOne.isInGuiding = false

-- 能否指引
function TaskGuideOne:canGuide()
...
-- 是否完成
function TaskGuideOne:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function TaskGuideOne:recoverInterrupt()
	if globaldata.level < self.level then
		return false
	end
...
-- 中断处理
function TaskGuideOne:recoverInterrupt_PVE()
	if globaldata.level < self.level then
		return false
	end
	if GuideSystem:getStepByGuideType(guideType) > 2 then -- 第一关没通关继续指引
...
function TaskGuideOne:stop()
	local function doStop()
		...
		-- 设置完成
		GuideSystem:setFinishByGuideType(guideType, 1)
		AnySDKManager:td_task_Complete("新手".."TaskGuideOne")
...
-- 指引操作
function TaskGuideOne:step(stepCount, touchRect, callBackFunc, callBackFunc2)
    ...
    local function doGuide()
    	...
    	if 1 == stepCount then
    		AnySDKManager:td_task_begin("新手".."TaskGuideOne")
...
-- 第一步,进入英雄界面
function TaskGuideOne:create1Layer(touchRect)
...
-- 第一步,进入英雄界面
function TaskGuideOne:create2Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,CG对话
function TaskGuideOne:create3Layer(callBackFunc)
	local layer = GuideSystem:createGuideCGLayer(221, callBackFunc)
