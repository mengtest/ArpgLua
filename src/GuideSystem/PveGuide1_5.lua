...
-- 指引层
PveGuide1_5.guideLayer = nil

-- 处于指引过程中
PveGuide1_5.isInGuiding = false

-- 能否指引
function PveGuide1_5:canGuide()
	...
	-- 前置指引必须完成
	if not PveGuideFive:isFinished() then
		return false
	end
...
-- 是否完成
function PveGuide1_5:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function PveGuide1_5:recoverInterrupt()
	-- 等级必须足够
	if globaldata.level < self.level then
		return false
	end
	...
	if globaldata:isSectionOpened(1, 5, 1) then
		if not globaldata:isChapterFinished(1, 5, 1) then -- 第三关没通关继续指引
...
-- 中断处理
function PveGuide1_5:recoverInterrupt_PVE()
	-- 等级必须足够
	if globaldata.level < self.level then
		return false
	end
...
function PveGuide1_5:stop()
	local function doStop()
		...
		-- 设置完成
		GuideSystem:setFinishByGuideType(guideType, 1)
		AnySDKManager:td_task_Complete("新手".."PveGuide1_5")
...
-- 指引操作
function PveGuide1_5:step(stepCount, touchRect, callBackFunc, callBackFunc2)
    ...
    local function doGuide()
    	...
    	if 1 == stepCount then
    		AnySDKManager:td_task_begin("新手".."PveGuide1_5")
...
-- 第一步,进入英雄界面
function PveGuide1_5:create1Layer(touchRect)
...
-- 第一步,进入英雄界面
function PveGuide1_5:create2Layer(callBackFunc)
...
-- 第二步,CG对话
function PveGuide1_5:create3Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuide1_5:create4Layer(touchRect, callBackFunc2)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuide1_5:create5Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
