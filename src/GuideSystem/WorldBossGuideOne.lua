...
WorldBossGuideOne.level = 22

-- 指引层
WorldBossGuideOne.guideLayer = nil

-- 动画1
WorldBossGuideOne.animNode1 = nil

-- 动画2
WorldBossGuideOne.animNode2 = nil

-- 处于指引过程中
WorldBossGuideOne.isInGuiding = false

-- 软指引
function WorldBossGuideOne:doHomeGuide()
	-- 完成不指引
	...
	if not self.animNode2 then
		...
		window.mRootWidget:getChildByName("Image_WorldBoss"):getChildByName("Panel_Open_Animation"):addChild(self.animNode2:getRootNode(), 100)
...
-- 软指引
function WorldBossGuideOne:doHomeGuideDestroy()
...
-- 能否指引
function WorldBossGuideOne:canGuide()
...
-- 是否完成
function WorldBossGuideOne:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function WorldBossGuideOne:recoverInterrupt()
	if globaldata.level < self.level then
		return false
	end
...
-- 设置完成
function WorldBossGuideOne:stop()
	local function doStop()
		...
		AnySDKManager:td_task_Complete("新手".."WorldBossGuideOne")
...
-- 指引操作
function WorldBossGuideOne:step(stepCount, touchRect, callBackFunc, callBackFunc2)
    ...
    local function doGuide()
    	...
    	if 1 == stepCount then
    		AnySDKManager:td_task_begin("新手".."WorldBossGuideOne")
...
-- 第一步,进入英雄界面
function WorldBossGuideOne:create1Layer(callBackFunc)
	local layer = GuideSystem:createGuideCGLayer(229, callBackFunc)
