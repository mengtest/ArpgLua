-- Name: 	LevelRewardGuideOnePointFive
-- Func：	等级奖励指引1
...
LevelRewardGuideOnePointFive = {}

LevelRewardGuideOnePointFive.level = 7

-- 指引层
LevelRewardGuideOnePointFive.guideLayer = nil

-- 处于指引过程中
LevelRewardGuideOnePointFive.isInGuiding = false

-- 能否指引
function LevelRewardGuideOnePointFive:canGuide()
...
-- 是否完成
function LevelRewardGuideOnePointFive:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function LevelRewardGuideOnePointFive:recoverInterrupt()
...
-- 中断处理
function LevelRewardGuideOnePointFive:recoverInterrupt_PVE()
	if globaldata.level < self.level then -- 小于5级不恢复
		return false
	end
	if not self:isFinished() then -- 没完成就继续指引
...
-- 暂停
function LevelRewardGuideOnePointFive:pause()
	-- 清理
...
-- 设置完成
function LevelRewardGuideOnePointFive:stop()
	...
	local function doStop()
		...
		-- 设置完成
		GuideSystem:setFinishByGuideType(guideType, 1)
		AnySDKManager:td_task_Complete("新手".."LevelRewardGuideOnePointFive")
...
-- 指引操作
function LevelRewardGuideOnePointFive:step(stepCount, touchRect, callBackFunc, callBackFunc2)
...
-- 第一步,进入英雄界面
function LevelRewardGuideOnePointFive:create1Layer(touchRect)
...
-- 第一步,进入英雄界面
function LevelRewardGuideOnePointFive:create2Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
