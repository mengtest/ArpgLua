...
-- 指引层
PveGuideOne.guideLayer = nil

-- 处于指引过程中
PveGuideOne.isInGuiding = false

-- 能否指引
function PveGuideOne:canGuide()
	...
	-- 前置指引必须完成
	if not HomeGuideOne:isFinished() then
		return false
	end
...
-- 是否完成
function PveGuideOne:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function PveGuideOne:recoverInterrupt()
...
-- 中断处理
function PveGuideOne:recoverInterrupt_PVE()
	if not globaldata:isChapterFinished(1, 1, 1) then -- 第一关没通关继续指引
		GuideSystem:setFinishByGuideType(guideType, 0)
...
-- 指引操作
function PveGuideOne:step(stepCount, touchRect, callBackFunc, callBackFunc2)
...
-- 第一步,进入英雄界面
function PveGuideOne:create1Layer(touchRect)
...
-- 第一步,进入英雄界面
function PveGuideOne:create2Layer(callBackFunc)
	local layer = GuideSystem:createGuideCGLayer(203, callBackFunc)
	GuideSystem:setStepByGuideType(guideType, 2)
	return layer
end

-- 第二步,CG对话
function PveGuideOne:create3Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuideOne:create4Layer(touchRect, callBackFunc2)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuideOne:create5Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
