...
-- 指引层
PveGuide1_7.guideLayer = nil

-- 处于指引过程中
PveGuide1_7.isInGuiding = false

-- 能否指引
function PveGuide1_7:canGuide()
	...
	-- 前置指引必须完成
	if not PveGuide1_6:isFinished() then
		return false
	end
...
-- 是否完成
function PveGuide1_7:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function PveGuide1_7:recoverInterrupt()
	-- 等级必须足够
	if globaldata.level < self.level then
		return false
	end
	...
	if globaldata:isSectionOpened(1, 7, 1) then
		if not globaldata:isChapterFinished(1, 7, 1) then -- 第三关没通关继续指引
...
-- 中断处理
function PveGuide1_7:recoverInterrupt_PVE()
	-- 等级必须足够
	if globaldata.level < self.level then
		return false
	end
...
function PveGuide1_7:stop()
	local function doStop()
		...
		-- 设置完成
		GuideSystem:setFinishByGuideType(guideType, 1)
		AnySDKManager:td_task_Complete("新手".."PveGuide1_7")
...
-- 指引操作
function PveGuide1_7:step(stepCount, touchRect, callBackFunc, callBackFunc2)
    ...
    local function doGuide()
    	...
    	if 1 == stepCount then
    		AnySDKManager:td_task_begin("新手".."PveGuide1_7")
...
-- 第一步,进入英雄界面
function PveGuide1_7:create1Layer(touchRect)
...
-- 第一步,进入英雄界面
function PveGuide1_7:create2Layer(callBackFunc)
...
-- 第二步,CG对话
function PveGuide1_7:create3Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuide1_7:create4Layer(touchRect, callBackFunc2)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuide1_7:create5Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
