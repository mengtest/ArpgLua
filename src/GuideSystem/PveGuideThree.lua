...
-- 指引层
PveGuideThree.guideLayer = nil

-- 处于指引过程中
PveGuideThree.isInGuiding = false

-- 能否指引
function PveGuideThree:canGuide()
	...
	-- 前置指引必须完成
	if not PveGuideTwo:isFinished() then
		return false
	end
...
-- 是否完成
function PveGuideThree:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function PveGuideThree:recoverInterrupt()
	if globaldata:isSectionOpened(1, 3, 1) then
		if not globaldata:isChapterFinished(1, 3, 1) then -- 第三关没通关继续指引
...
-- 中断处理
function PveGuideThree:recoverInterrupt_PVE()
	if globaldata:isSectionOpened(1, 3, 1) then
		if not globaldata:isChapterFinished(1, 3, 1) then -- 第三关没通关继续指引
			GuideSystem:setFinishByGuideType(guideType, 0)
...
function PveGuideThree:stop()
	local function doStop()
		...
		-- 设置完成
		GuideSystem:setFinishByGuideType(guideType, 1)
		AnySDKManager:td_task_Complete("新手".."PveGuideThree")
...
-- 指引操作
function PveGuideThree:step(stepCount, touchRect, callBackFunc, callBackFunc2)
...
-- 第一步,进入英雄界面
function PveGuideThree:create1Layer(touchRect)
...
-- 第一步,进入英雄界面
function PveGuideThree:create2Layer(callBackFunc)
	local layer = GuideSystem:createGuideCGLayer(205, callBackFunc)
	GuideSystem:setStepByGuideType(guideType, 2)
	return layer
end

-- 第二步,CG对话
function PveGuideThree:create3Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuideThree:create4Layer(touchRect, callBackFunc2)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuideThree:create5Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
