...
BlackMarketGuide.level = 36

-- 指引层
BlackMarketGuide.guideLayer = nil

-- 动画1
BlackMarketGuide.animNode1 = nil

-- 动画2
BlackMarketGuide.animNode2 = nil

-- 处于指引过程中
BlackMarketGuide.isInGuiding = false

-- 软指引
function BlackMarketGuide:doHomeGuide()
	-- 完成不指引
	...
	if not self.animNode2 then
		...
		window.mRootWidget:getChildByName("Image_BlackMarket"):getChildByName("Panel_Open_Animation"):addChild(self.animNode2:getRootNode(), 100)
...
-- 软指引
function BlackMarketGuide:doHomeGuideDestroy()
...
-- 能否指引
function BlackMarketGuide:canGuide()
...
-- 是否完成
function BlackMarketGuide:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function BlackMarketGuide:recoverInterrupt()
	if globaldata.level < self.level then
		return false
	end
...
-- 设置完成
function BlackMarketGuide:stop()
	local function doStop()
		...
		AnySDKManager:td_task_Complete("新手".."BlackMarketGuide")
...
-- 指引操作
function BlackMarketGuide:step(stepCount, touchRect, callBackFunc, callBackFunc2)
    ...
    local function doGuide()
    	if 1 == stepCount then
    		AnySDKManager:td_task_begin("新手".."BlackMarketGuide")
...
-- 第一步,进入英雄界面
function BlackMarketGuide:create1Layer(callBackFunc)
	local layer = GuideSystem:createGuideCGLayer(236, callBackFunc)
