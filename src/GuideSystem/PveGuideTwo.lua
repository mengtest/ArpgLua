...
-- 指引层
PveGuideTwo.guideLayer = nil

-- 处于指引过程中
PveGuideTwo.isInGuiding = false

-- 能否指引
function PveGuideTwo:canGuide()
	...
	-- 前置指引必须完成
	if not PveGuideOne:isFinished() then
		return false
	end
...
-- 是否完成
function PveGuideTwo:isFinished()
	return 1 == GuideSystem:getFinishByGuideType(guideType)
end

-- 中断处理
function PveGuideTwo:recoverInterrupt()
	if globaldata:isSectionOpened(1, 2, 1) then
		if not globaldata:isChapterFinished(1, 2, 1) then -- 第二关没通关继续指引
...
-- 中断处理
function PveGuideTwo:recoverInterrupt_PVE()
	if globaldata:isSectionOpened(1, 2, 1) then
		if not globaldata:isChapterFinished(1, 2, 1) then -- 第二关没通关继续指引
			GuideSystem:setFinishByGuideType(guideType, 0)
...
function PveGuideTwo:stop()
	local function doStop()
		...
		-- 设置完成
		GuideSystem:setFinishByGuideType(guideType, 1)
		AnySDKManager:td_task_Complete("新手".."PveGuideTwo")
...
-- 指引操作
function PveGuideTwo:step(stepCount, touchRect, callBackFunc, callBackFunc2)
    ...
    local function doGuide()
    	...
    	if 1 == stepCount then
    		AnySDKManager:td_task_begin("新手".."PveGuideTwo")
...
-- 第一步,进入英雄界面
function PveGuideTwo:create1Layer(touchRect)
...
-- 第一步,进入英雄界面
function PveGuideTwo:create2Layer(callBackFunc)
	local layer = GuideSystem:createGuideCGLayer(204, callBackFunc)
	GuideSystem:setStepByGuideType(guideType, 2)
	return layer
end

-- 第二步,CG对话
function PveGuideTwo:create3Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuideTwo:create4Layer(touchRect, callBackFunc2)
	local layer = GuideSystem:createGuideLayer(touchRect)
...
-- 第二步,进入技能标签
function PveGuideTwo:create5Layer(touchRect)
	local layer = GuideSystem:createGuideLayer(touchRect)
